	...
	EditArea.prototype.toggle_full_screen= function(to){
		...
		frame = p.document.getElementById("template_editor_content");
		...
		{	// toogle on fullscreen		
			...
			t.fullscreen['old_editor_height']= frame.style.height;
			if(t.isOpera){
			...
			p.window['fn_adjust_template_list'](html.clientHeight);
			...
			frame.style.top= "-"+(p.calculeOffsetTop(frame) + 3)+"px";
			frame.style.left= "-"+(p.calculeOffsetLeft(frame) + 3)+"px";
		...
		{	// toogle off fullscreen
			...
			frame.style.height= t.fullscreen['old_editor_height'];
			p.window['fn_adjust_template_list'](parseInt(t.fullscreen['old_editor_height']));
			...
			t.textarea.disabled = false; //special for IE
